...
export default function SubmissionsByHandle() {
  ...
  useEffect(() => {
    if (!userhandle) { navigate('/Login'); return; }
    axios.get(`${API_BASE_URL}/api/submissions/readbyhandle`)
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-24 pb-16">
          ...
          <div className="mb-6">
            <p className="text-xs font-bold tracking-widest uppercase mb-1" style={{ color: '#61afef' }}>
              CodeJudge · <span className="font-mono">@{userhandle}</span>
            </p>
          ...
          <div className="rounded-2xl border overflow-hidden"
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm" style={{ borderCollapse: 'collapse', minWidth: 540 }}>
                <thead>
                  <tr style={{ background: '#1c2128', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                    {['#', 'Time', 'Problem', 'Language', 'Status', 'Exec Time', 'Code'].map((h, i) => (
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col gap-0.5">
                          <Link to={`/ProblemDescription/${sub.PID}`}
                            ...
                            style={{ color: hoveredRow === sub._id ? '#61afef' : '#c9d1d9', textDecoration: 'none' }}>
                            {sub.ProblemName || sub.PID}
                          </Link>
                          <span className="text-xs font-mono" style={{ color: '#56b6c2' }}>{sub.PID}</span>
                        </div>
                      </td>
